...
parent 26th, while CT-RAG \texttt{WHY} ranks it first using explicit
...
alternatives are returned. A secondary matched-FPR degradation study
...
\textbf{Table 1. Absolute rank of the true causal parent under
adversarial semantic and temporal geometry.}
...
\bottomrule\noalign{}
\endlastfoot
GlobalFix & \textbf{0.90} & 10 & 0.596 & \textbf{No} & excluded \\
...
point estimate. \texttt{ProviderFallback} remains clearly separated from
both.

\textbf{Table 3. Top-1 recovery comparison.}
...
CT-RAG \texttt{RECOVERY} & \textbf{ProviderFallback} & \textbf{1.0} \\
\end{longtable}
...
\subsubsection{Remaining isolation
check}\label{remaining-isolation-check}

One validation remains desirable before camera-ready submission. The
same fixture should be mutated so that \texttt{GlobalFix} becomes
causally reachable from \texttt{retry}. If causal reachability is
strictly a gate rather than an undeclared penalty, the now-reachable
9/10 branch should be eligible and, all else controlled, its Wilson
lower bound (0.596) should exceed \texttt{ProviderFallback} (0.490).
This mutation test is \textbf{not yet reported as executed} in CI \#210
and is preregistered here as a follow-up validation.
...
demoted because the detectors did not share matched false-positive
operating points. Under the primary FPR-matched ablation, threshold
(\theta=0.10) with two sustained windows detects behavioral change on
day 8 and infrastructure change on day 9, yielding a \textbf{+1-day
relative lead} with FPR gap 0.
...
\item
  all 28 cells are FPR matched with maximum FPR gap 0 in the controlled
  null generator;
\item
